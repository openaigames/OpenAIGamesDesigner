...
def scaffold_project(args):
    ...
    pending = [
        ("游戏设计", "核心交互、规则与边界；不从模糊目标默认加入闪避、体力或其他机制。", "明确首个核心交互或获授权采用默认方案时，记录规则及其依据。"),
        ("美术与表现", "视觉方向、角色、场景、动画、VFX、声音和 UI；暂无已核实的资产记录。", "制作可见资产前明确本轮模型/动作与占位范围或选材委托；收到参考或使用素材后按表登记来源、用途、状态与限制，最终方向可以继续待定。"),
        ("技术与工程", "引擎、版本、框架、工程入口与运行方式；当前尚未核实。", "环境检查或实施后立即记录技术事实；未决定的选型另列待定，不等待用户批准才记事实。"),
        ("风险与验证", "首个实验问题、观察方法与验收依据；当前没有执行或试玩证据。", "设计形成时补假设；每次检查后关联真实结果、限制及下一步，失败或中断也要记录。"),
    ]
