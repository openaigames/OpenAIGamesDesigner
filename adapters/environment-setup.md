...
| UE 的 GAS 能力制作（可选） | 选型已确定；目标版本 GameplayAbilities 插件；C++ 实现需要匹配工具链 | UE 工程 Plugins / .uproject 核对插件，C++ 模块按实际 API 配置依赖；不需完整样例工程 | 实际 ASC/属性/能力初始化、授予、触发及清理；启用插件或 MCP 不代表完成 |
...
并非每套 MCP 都需要引擎插件。例如本次调研的 Unreal 候选需要 C++ 插件，Godot 候选通过 Godot 可执行文件完成其支持的操作。使用提供方实际要求，不把 UE 的步骤套到所有引擎上。
...
机器上的绝对路径由每位使用者按实际情况填写；不要把维护者电脑的路径当作默认安装要求。密钥不写入会归档的命令参数和文档，使用工具自身凭据管理或环境变量名引用。
