...
- 检查现有 C++、Blueprint、配置、资源引用、地图入口与已有构建/测试方式。新能力/属性系统需要选型时读取 [能力系统选型](ability-system-choice.md)；角色动作按 [动作与动画接入](action-animation-integration.md)。已有系统原位接入，不默认以样例工程重建。工程规则以当前需求与已有实现为依据。
