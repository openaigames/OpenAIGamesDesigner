...
- 检查现有 C++、Blueprint、配置、资源引用、地图入口与已有构建/测试方式。只有确实使用 GAS 或 Lyra 时才读取相应专项参考。用户的另一款游戏不是此项目的默认规则来源。
...
这是方法参考。当前 `game_workflow.py` 的 UE 路线支持工程结构检查、游戏启动和项目配置命令，配置方式见共享工具包 `adapters/README.md`；通用编辑器资产连接尚未接入。不要对 UE 调用 Godot 参数或创建 `project.godot`；使用实际可用的 UE/项目工具，并把缺失能力记录到任务。未安装引擎或缺少工程时仍可完成有依据的设计与准备，但不得伪称可运行。
